...
def minmax(board, depth, isMaximazing,alpha,beta):
    ...
    if isMaximazing:
        ...
        for pos in range(len(board)):
            ...
            if board[pos] == '-':
                board[pos] = "O"
                score = minmax(board, 0, False,alpha,beta)
                board[pos] = "-"
                if score > bestScore:
                    bestScore = score
                if (bestScore > alpha):
                    alpha = bestScore
                if (alpha>=beta):
                    return (bestScore)  #prune the tree
        return bestScore
    else:
        bestScore = 800
...
RetsLabel = Label(HomeFrame, text="Tic Tac Toe using min-max & alphaBeta", font=text_font, bg=bg_color)
